...
exports.addCheckedFunction = function (env) {
  env.addGlobal('checked', function (name, value) {
    ...
    // Use passed data, or string keys or object notation to support:
    ...
    if (Array.isArray(storedValue)) {
      if (_.isArray(storedValue[0])){
        if (storedValue[0].indexOf(value) !== -1) {
          checked = 'checked'
        }
      }
      else if (storedValue.indexOf(value) !== -1) {
        checked = 'checked'
      }
    } else {
